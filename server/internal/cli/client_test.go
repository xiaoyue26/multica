...
func TestDownloadFile(t *testing.T) {
	...
	t.Run("sub-path BaseURL: relative download path preserves sub-path prefix", func(t *testing.T) {
		var gotPath string
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			gotPath = r.URL.Path
			w.Write([]byte("data"))
	...
	t.Run("sub-path BaseURL with trailing slash: relative download path preserves sub-path prefix", func(t *testing.T) {
		var gotPath string
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			gotPath = r.URL.Path
			w.Write([]byte("data"))
	...
	t.Run("sub-path BaseURL: relative path without leading slash is handled correctly", func(t *testing.T) {
		var gotPath string
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			gotPath = r.URL.Path
			w.Write([]byte("data"))
